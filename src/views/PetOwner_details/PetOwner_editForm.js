...
const EditPetOwnerForm = () => {
  ...
  useEffect(() => {
    const fetchPetOwnerById = async () => {
      ...
      const jsonResp = await petOwnerResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_PETOWNER}(${formData.OwnerId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated PetOwner', {
        variant: 'success',
      });
      navigate({ pathname: '/petOwners' });
    } else {
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/petOwners' })}
              >
                Cancel
              </Button>
            </div>
...
export default EditPetOwnerForm;
